...
const MovieCard: React.FC<MovieCardProps> = ({ movie, api, index, onClick }) => {
    ...
    return (
        ...
        >
            <div className="relative bg-gray-900 rounded-lg overflow-hidden shadow-lg transition-all duration-300 hover:scale-105"
                style={{
...
const SeriesPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            ...
            try {
                ...
                const seriesLibrary = libraries.find((lib: any) => lib.CollectionType === 'tvshows');
                if (!seriesLibrary) throw new Error('No series library found');

                const seriesItems = await getAllLibraryItems(api, userId, seriesLibrary.Id, { IncludeItemTypes: ['Series'] });
                // Randomize the order for more interesting masonry layout
                setSeries(seriesItems.sort(() => Math.random() - 0.5));
            } catch (err: any) {
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-black">
                <div className="text-xl text-white">Loading series...</div>
            </div>
        );
    }
    ...
    const masonryColumns = Array.from({ length: columns }, (_, i) =>
        filteredSeries.filter((_, index) => index % columns === i)
    );
